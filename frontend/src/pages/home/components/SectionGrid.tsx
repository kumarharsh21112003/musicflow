...
const LoadingSkeleton = () => (
	<div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4'>
		{[1, 2, 3, 4, 5].map(i => (
			<div key={i} className='bg-neutral-900/40 p-4 rounded-lg animate-pulse'>
	...
	return (
		<div className='mb-8'>
			<div className='flex items-center justify-between mb-4 px-1'>
				...
				<button className='text-xs font-bold text-neutral-400 hover:text-white uppercase tracking-wider'>
					See all
				</button>
			</div>
				...
				{songs.map((song) => {
					...
					return (
						<div
							...
							className='min-w-[160px] md:min-w-0 bg-neutral-950/40 p-3 rounded-md hover:bg-neutral-900/40 transition-all group cursor-pointer snap-start border border-white/5'
						>
							<div className='relative mb-3'>
								<div className='aspect-square rounded shadow-2xl overflow-hidden bg-neutral-900'>
									<img
								...
								<Button
									size="icon"
									className={`absolute bottom-2 right-2 w-10 h-10 bg-orange-500 hover:bg-orange-400 hover:scale-105 transition-all rounded-full
							...
							<h3 className={`font-bold text-sm mb-0.5 truncate ${isCurrentSong ? 'text-orange-400' : 'text-white'}`}>
								{song.title}
							</h3>
							<p className='text-[12px] text-neutral-400 font-medium truncate opacity-80'>{song.artist}</p>
						</div>
					);
				})}
			</div>
